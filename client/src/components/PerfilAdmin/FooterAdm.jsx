import React from "react";
import "../estilos/Footer.css";

const FooterAdm = () => {
  return (
    <div className="main-FooterAdm">
      <h4 className="h4-form">WEB DEVELOPERS</h4>
      <div className="devs-container">
        <div class="our-team">
          <div class="pic">
            <img src="https://media-exp1.licdn.com/dms/image/C4D03AQGafSUNe1xkRQ/profile-displayphoto-shrink_800_800/0/1522107920366?e=1668038400&v=beta&t=hZLo5KbMBWIcwbWx8cMU8V_CO7fk8LOPOy_MdoU1bX8" />
          </div>
          {/* <h3 class="title">Web Developer</h3> */}
          <span class="post">Andreas Heller</span>
          <ul class="social">
            <li>
              <a href="https://twitter.com/TheAndiHaller" target="_blank">
                <i className="fab fa-twitter"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.instagram.com/theandihaller/"
                target="_blank"
              >
                <i className="fab fa-instagram"></i>
              </a>
            </li>
            <li>
              <a href="https://github.com/TheAndiHaller" target="_blank">
                <i className="fab fa-github"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.linkedin.com/in/andreas-haller-schade-558097105/"
                target="_blank"
              >
                <i className="fab fa-linkedin"></i>
              </a>
            </li>
          </ul>
        </div>

        <div class="our-team">
          <div class="pic">
            <img src="https://ca.slack-edge.com/TPRS7H4PN-U03B01CQ2NT-44a9d4821560-512" />
          </div>
          {/* <h3 class="title">Marko Schneider</h3> */}
          <span class="post">Marko Schneider</span>
          <ul class="social">
            <li>
              <a
                href="https://www.facebook.com/Marko.schneiders"
                target="_blank"
              >
                <i className="fab fa-facebook"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.instagram.com/marko_schneider"
                target="_blank"
              >
                <i className="fab fa-instagram"></i>
              </a>
            </li>
            <li>
              <a href="https://github.com/markon1i" target="_blank">
                <i className="fab fa-github"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.linkedin.com/in/marko-schneider"
                target="_blank"
              >
                <i className="fab fa-linkedin"></i>
              </a>
            </li>
          </ul>
        </div>

        <div class="our-team">
          <div class="pic">
            <img src="https://ca.slack-edge.com/TPRS7H4PN-U0387NWK7QR-dafca4021140-512" />
          </div>
          {/*  <h3 class="title">Gaspar Muñoz</h3> */}
          <span class="post">Gaspar Muñoz</span>
          <ul class="social">
            <li>
              <a href="https://twitter.com/Gaspar_gm" target="_blank">
                <i className="fab fa-twitter"></i>
              </a>
            </li>
            <li>
              <a href="https://www.instagram.com/gaspar.mnz" target="_blank">
                <i className="fab fa-instagram"></i>
              </a>
            </li>
            <li>
              <a href="https://github.com/gaspar1407" target="_blank">
                <i className="fab fa-github"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.linkedin.com/in/gaspar-mu%C3%B1oz-7a9825239/"
                target="_blank"
              >
                <i className="fab fa-linkedin"></i>
              </a>
            </li>
          </ul>
        </div>

        <div class="our-team">
          <div class="pic">
            <img src="https://media-exp1.licdn.com/dms/image/C4E03AQHYDAoAkLeg6w/profile-displayphoto-shrink_800_800/0/1645985790536?e=1668038400&v=beta&t=dBZti0oDKrho2kfK1MaudFBg_qgDs96_D-1W9qokaL8" />
          </div>
          {/*  <h3 class="title">Carolina Mejía</h3> */}
          <span class="post">Carolina Mejía</span>
          <ul class="social">
            <li>
              <a
                href="https://www.facebook.com/profile.php?id=100014969531221"
                target="_blank"
              >
                <i className="fab fa-facebook"></i>
              </a>
            </li>
            <li>
              <a href="https://www.instagram.com/carol_3116" target="_blank">
                <i className="fab fa-instagram"></i>
              </a>
            </li>
            <li>
              <a href="https://github.com/CAROLMEJIA" target="_blank">
                <i className="fab fa-github"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.linkedin.com/in/carolina-mej%C3%ADa-zapata-433298233"
                target="_blank"
              >
                <i className="fab fa-linkedin"></i>
              </a>
            </li>
          </ul>
        </div>

        <div class="our-team">
          <div class="pic">
            <img src="https://ca.slack-edge.com/TPRS7H4PN-U03CJVDB7BJ-e522cd1833c2-512" />
          </div>
          {/* <h3 class="title">Agustin Viera</h3> */}
          <span class="post">Agustin Viera</span>
          <ul class="social">
            <li>
              <a
                href="https://mobile.twitter.com/agusttinviera"
                target="_blank"
              >
                <i className="fab fa-twitter"></i>
              </a>
            </li>
            <li>
              <a href="https://www.instagram.com/agusviera__" target="_blank">
                <i className="fab fa-instagram"></i>
              </a>
            </li>
            <li>
              <a href="https://github.com/VieraAgus98" target="_blank">
                <i className="fab fa-github"></i>
              </a>
            </li>
            <li>
              <a
                href=" https://www.linkedin.com/in/agustinvieradev"
                target="_blank"
              >
                <i className="fab fa-linkedin"></i>
              </a>
            </li>
          </ul>
        </div>

        <div class="our-team">
          <div class="pic">
            <img src="https://media-exp1.licdn.com/dms/image/C4E03AQH_RxzysF5uag/profile-displayphoto-shrink_800_800/0/1572271033234?e=1668038400&v=beta&t=YO3gq_IHrNNI_nBTyejB1D4DCkfNNNxEk5wPcYD2EXY" />
          </div>
          {/* <h3 class="title">José Correa</h3> */}
          <span class="post">José Correa</span>
          <ul class="social">
            <li>
              <a
                href="https://www.facebook.com/profile.php?id=100009434297833"
                target="_blank"
              >
                <i className="fab fa-facebook"></i>
              </a>
            </li>
            <li>
              <a href="https://www.instagram.com/mishino100/" target="_blank">
                <i className="fab fa-instagram"></i>
              </a>
            </li>
            <li>
              <a href="https://github.com/mishino123" target="_blank">
                <i className="fab fa-github"></i>
              </a>
            </li>
            <li>
              <a
                href="https://www.linkedin.com/in/jose-israel-correa-cortes-00651179"
                target="_blank"
              >
                <i className="fab fa-linkedin"></i>
              </a>
            </li>
          </ul>
        </div>
      </div>
      <div className="derechosAdm">
        <p>2022 HENRY FITNESS Argentina. Todos los derechos reservados</p>
      </div>
    </div>
  );
};

export default FooterAdm;
